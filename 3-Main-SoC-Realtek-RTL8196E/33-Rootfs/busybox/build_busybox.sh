...
# --- HOLD_RF_RESET -----------------------------------------------------------
# Same flag and values as the other build scripts.  When set, patch
# 910-init-builtin-fallback-inittab.patch is applied and a fallback inittab
# table is embedded into the init binary (generated from skeleton/etc/inittab
# with respawn/askfirst filtered out — on rf-hold builds /etc/inittab is a
# symlink that is unreadable during boot, and nothing may spawn on ttyS0
# before init re-reads the real table).  Without the flag, vanilla busybox
# defaults are kept.
...
# If the HOLD_RF_RESET flag changed since the last build, the patch set
# differs (910-* applied or not) — force a clean re-extract.
RFHOLD_STATE="${SRC_DIR}/.rfhold_state"
if [ -d "${SRC_DIR}" ] && \
   [ "${RF_HOLD}" != "$(cat "${RFHOLD_STATE}" 2>/dev/null || echo 0)" ]; then
  echo "♻️  HOLD_RF_RESET flag changed — re-extracting ${SRC_DIR}"
  rm -rf "${SRC_DIR}"
fi
...
if [ ! -f "${PATCH_MARKER}" ] || [ "${NEED_PATCH}" -eq 1 ]; then
  ...
  if [ -d "$PATCH_DIR" ]; then
    ...
    for p in "$PATCH_DIR"/*.patch; do
      ...
      case "$name" in
        910-*)
          # rf-hold only: needs the generated builtin_inittab.h header.
          if [ "$RF_HOLD" != "1" ]; then
            echo "  ⏭️  $name (skipped — HOLD_RF_RESET not enabled)"
            continue
          fi
          ;;
      esac
...
echo "$RF_HOLD" > "${RFHOLD_STATE}"

# rf-hold builds: embed the fallback inittab (see patch 910).  Derived from
# the rootfs skeleton's stock inittab; respawn/askfirst entries are removed —
# the getty is owned by /userdata/etc/inittab after the boot-time table
# reload, and nothing may spawn on ttyS0 while that symlink still dangles.
if [ "$RF_HOLD" = "1" ]; then
  INITTAB_SRC="${ROOTFS_DIR}/etc/inittab"
  if [ ! -f "$INITTAB_SRC" ]; then
    echo "ERROR: skeleton inittab not found at $INITTAB_SRC" >&2
    exit 1
  fi
  echo "📝 Generating ${SRC_DIR}/init/builtin_inittab.h from skeleton/etc/inittab..."
  {
    echo "/* Generated by build_busybox.sh — DO NOT EDIT."
    echo "   Source: 33-Rootfs/skeleton/etc/inittab with respawn/askfirst"
    echo "   entries removed (getty is owned by /userdata/etc/inittab). */"
    echo "static const char *const builtin_inittab_lines[] = {"
    awk -F: '
      /^[ \t]*#/ || /^[ \t]*$/ { next }
      $3 == "respawn" || $3 == "askfirst" { next }
      { gsub(/\\/, "\\\\"); gsub(/"/, "\\\""); printf "\t\"%s\",\n", $0 }
    ' "${INITTAB_SRC}"
    echo "    NULL,"
    echo "};"
  } > "${SRC_DIR}/init/builtin_inittab.h"
fi
...
EXTRA_CFLAGS=""
[ "$RF_HOLD" = "1" ] && EXTRA_CFLAGS="-DINITTAB_BUILTIN_FALLBACK"
make CROSS_COMPILE="${CROSS_COMPILE}" EXTRA_CFLAGS="$EXTRA_CFLAGS"
